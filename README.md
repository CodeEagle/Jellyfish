...
目录包含兼容 `lzcat-trigger` 的两个工作流：

- `.github/workflows/update-image.yml`: 拉取上游最新提交，构建并推送 `ghcr.io/<owner>/jellyfish:<target-repo-commit-sha>`
- `.github/workflows/trigger-build.yml`: 调用 `CodeEagle/lzcat-trigger` 统一完成镜像复制、manifest 回写和 `.lpk` 构建
